...
static u32 cookie_sha_hash(__be32 saddr, __be32 daddr, __be16 sport, __be16 dport,
		       u32 count, int c)
{
                        ...
                        u32 count, int c)
{
	...
	if (sysctl_ip_vs_synproxy_cookie_hash == COOKIE_HASH_SHA) {
		return cookie_sha_hash(saddr, daddr, sport, dport, count, c);
	}
	...
	if (sysctl_ip_vs_synproxy_cookie_hash == COOKIE_HASH_CRC) {
		__u32 *tmp = __get_cpu_var(ipv4_cookie_scratch);
		size_t secret_bytes = CRC_SECRET_BYTES;
		
		if (unlikely(secret_bytes > sizeof(net_secret[c])))
			secret_bytes = sizeof(net_secret[c]);

		tmp[0] = hash[0];
		tmp[1] = hash[1];
		tmp[2] = hash[2];
		tmp[3] = hash[3];
		memcpy(tmp + 4, net_secret[c], secret_bytes);
		return crc32c(hash[0], tmp, 4 + secret_bytes);
	}
