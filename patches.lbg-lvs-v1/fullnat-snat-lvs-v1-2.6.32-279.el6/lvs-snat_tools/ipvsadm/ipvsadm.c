...
#define CMD_ADDLADDR_ZONE		(CMD_NONE+20)
#define CMD_DELLADDR_ZONE		(CMD_NONE+21)
#define CMD_MAX			CMD_DELLADDR_ZONE
...
/*ADDLAZO*/ {'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', '+', 'x', '+'},
/*DELLAZO*/ {'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x', '+', 'x', '+'},
};
...
static int list_laddrs_zone(ipvs_zone_t *zone , int with_title);
	      ...
	      unsigned int *options, unsigned int *format)
{
    ...
    { "add-laddr-zone", 'U', POPT_ARG_NONE, NULL, 'U', NULL, NULL },
		{ "del-laddr-zone", 'W', POPT_ARG_NONE, NULL, 'W', NULL, NULL },
	...
	case 'Q':
		...
		break;
  case 'U':
		set_command(&ce->cmd, CMD_ADDLADDR_ZONE);
		break;
	case 'W':
		set_command(&ce->cmd, CMD_DELLADDR_ZONE);
		break;
	case 'G':
...
static int process_options(int argc, char **argv, int reading_stdin)
{
  ...
  case CMD_ADDLADDR_ZONE:
		result = ipvs_add_laddr_zone(&ce.zone , &ce.laddr);
		break;

	case CMD_DELLADDR_ZONE:
		result = ipvs_del_laddr_zone(&ce.zone , &ce.laddr);
		break;

	case CMD_GETLADDR:
    if(options & OPT_SERVICE)
			result = list_laddrs(&ce.svc , 1);
		else if(options & OPT_ZONE)
			result = list_laddrs_zone(&ce.zone , 1);
		else
...
static void usage_exit(const char *program, const int exit_status)
{
    ...
    "  --add-laddr-zone  -U        add local address to zone\n"
		"  --del-laddr-zone  -W        del local address from zone\n"
...
static int list_laddrs_zone(ipvs_zone_t *zone , int with_title)
{
	...
	if (!(d = ipvs_get_laddrs_zone(entry))) {
		fprintf(stderr, "%s\n", ipvs_strerror(errno));
		free(entry);
		return -1;
	}	
...
static int list_all_laddrs(void)
{
  struct ip_vs_get_services *get_svc;
	...
	for (i = 0; i < get->num_zones; i++){
		if(!(d = ipvs_get_laddrs_zone(&(get->entrytable[i])))) {
			free(get);
			fprintf(stderr, "%s\n", ipvs_strerror(errno));
			return -1;
		}
  ...
  title_enable = 1;
  for (i = 0; i < get_svc->num_services; i++){
		if(!(d = ipvs_get_laddrs(&(get_svc->entrytable[i])))) {
			free(get);
      free(get_svc);
			fprintf(stderr, "%s\n", ipvs_strerror(errno));
			return -1;
