...
int ipvs_add_laddr_zone(ipvs_zone_t *zone, ipvs_laddr_t * laddr)
{
	ipvs_zoneladdr_t zoneladdr;
	ipvs_func = ipvs_add_laddr_zone;

#ifdef LIBIPVS_USE_NL
	if (try_nl) {
		struct nl_msg *msg = ipvs_nl_message(IPVS_CMD_NEW_LADDR_ZONE , 0);
		if (!msg) return -1;
...
nla_put_failure:
	...
	return setsockopt(sockfd, IPPROTO_IP, IP_VS_SO_SET_ADDLADDR_ZONE,
			  (char *)&zoneladdr, sizeof(zoneladdr));
}

int ipvs_del_laddr_zone(ipvs_zone_t *zone, ipvs_laddr_t * laddr)
{
	ipvs_zoneladdr_t zoneladdr;
	ipvs_func = ipvs_del_laddr_zone;

#ifdef LIBIPVS_USE_NL
	if (try_nl) {
		struct nl_msg *msg = ipvs_nl_message(IPVS_CMD_DEL_LADDR_ZONE , 0);
		if (!msg) return -1;
		if (ipvs_nl_fill_zone_attr(msg, zone))
			goto nla_put_failure;
		if (ipvs_nl_fill_laddr_attr(msg, laddr))
...
nla_put_failure:
	...
	return setsockopt(sockfd, IPPROTO_IP, IP_VS_SO_SET_DELLADDR_ZONE,
                          (char *)&zoneladdr, sizeof(zoneladdr));
}
...
struct ip_vs_get_laddrs *ipvs_get_laddrs_zone(ipvs_zone_entry_t *zone)
{
	struct ip_vs_get_laddrs 	*l;
	struct ip_vs_get_laddrs_kern 	*lk;
	socklen_t 			len;
	...
	ipvs_func = ipvs_get_laddrs_zone;
	...
	if (try_nl) {
		...
		msg = ipvs_nl_message(IPVS_CMD_GET_LADDR_ZONE , NLM_F_DUMP);
		if (!msg)
			goto ipvs_nl_laddr_failure;
...
ipvs_nl_laddr_failure:
	...
	if (getsockopt(sockfd, IPPROTO_IP,
		       IP_VS_SO_GET_LADDRS_ZONE, lk, &len) < 0) {
		free(l);
		free(lk);
		return NULL;
	}
...
const char *ipvs_strerror(int err)
{
    ...
    { ipvs_del_laddr_zone, ESRCH, "Zone not defined" },
		{ ipvs_del_laddr_zone, ENOENT, "No such Local address" },
		{ ipvs_get_laddrs, ESRCH, "Svc not defined" },
    { ipvs_get_laddrs_zone, ESRCH, "Zone not defined" },
